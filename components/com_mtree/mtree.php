...
switch ($task) {
	...
	case "fields.list":
		...
		break;
	case "map":
		showTreeMap( $cat_id, $limitstart, $option, $my );
		break;
	/* Default Main Index */
	case "listcats":
	case "home":
...
function showTreeMap( $cat_id, $limitstart, $option, $my ) {
	global $mtconf;
	...
	if ( !is_null($cat) ) {
		...
		$cache = JFactory::getCache('com_mtree');
		$cache->call( 'showTreeMap_cache', $cat, $limitstart, $option, $my );
...
function showTreeMap_cache( $cat, $limitstart, $option, $my ) {
	...
	if ( isset($cat->cat_published) && $cat->cat_published == 0 && $cat_id > 0 ) {
		...
		else
		{
			 ...
			 $sql = "SELECT l.*, cf.value as cordinate, cmf.value as showMap, cmfs.value as typeFlag, tlcat.cat_id AS tlcat_id, tlcat.cat_name AS tlcat_name, cl.*, cat.*, u.username AS username, u.name AS owner, img.filename AS link_image FROM #__mt_links AS l"
				...
				.	"\n LEFT JOIN #__mt_cfvalues AS cf ON cf.link_id = l.link_id and cf.cf_id='32' "
				.	"\n LEFT JOIN #__mt_cfvalues AS cmf ON cmf.link_id = l.link_id and cmf.cf_id='33' "
				.	"\n LEFT JOIN #__mt_cfvalues AS cmfs ON cmfs.link_id = l.link_id and cmfs.cf_id='30' "
		...
		if ( $cat_id == 0 || $cat->cat_usemainindex == 1 ) {
			...
			$savant->display( 'page_index_map.tpl.php' );
		} else {
...
function showTree_cache( $cat, $limitstart, $option, $my ) {
	...
	if ( isset($cat->cat_published) && $cat->cat_published == 0 && $cat_id > 0 ) {
		...
		# Page Navigation
		$database->setQuery( 'SELECT COUNT(*) FROM (#__mt_links AS l, #__mt_cl AS cl, #__mt_cfvalues as cf) WHERE l.link_published = 1 AND cf.link_id=l.link_id AND cf.value="Supplier" AND l.link_approved = 1 && cl.cat_id = ' . $database->quote($cat_id)
			. "\n AND ( l.publish_up = ".$database->Quote($nullDate)." OR l.publish_up <= '$now'  ) "
			. "\n AND ( l.publish_down = ".$database->Quote($nullDate)." OR l.publish_down >= '$now' ) "
			. "\n AND cl.link_id = l.link_id "
		);
		...
		else
		{
			...
			$sql = "SELECT l.*, tlcat.cat_id AS tlcat_id, tlcat.cat_name AS tlcat_name, cl.*, cat.*, u.username AS username, u.name AS owner, img.filename AS link_image FROM #__mt_links AS l"
				.	"\n LEFT JOIN #__mt_cfvalues as cf ON cf.link_id = l.link_id "
				.	"\n LEFT JOIN #__mt_cl AS cl ON cl.link_id = l.link_id "
				.	"\n LEFT JOIN #__users AS u ON u.id = l.user_id "
				.	"\n LEFT JOIN #__mt_cats AS cat ON cl.cat_id = cat.cat_id "
				.	"\n LEFT JOIN #__mt_cats AS tlcat ON tlcat.lft <= cat.lft AND tlcat.rgt >= cat.rgt AND tlcat.cat_parent =0 "
