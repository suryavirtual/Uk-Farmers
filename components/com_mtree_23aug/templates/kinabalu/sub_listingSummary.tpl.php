...
?>" data-link-id="<?php echo $link->link_id; ?>">
		...
		{
			/* new code for fetching logo images from uf_data */

		   $database = JFactory::getDbo();
		$linkId = $link->link_id;
        $usertypequery = ("SELECT value FROM jos_mt_cfvalues WHERE cf_id = '30' and  link_id='$linkId'");
        $database->setQuery($usertypequery);

        $usertypesdata = $database->loadObject();

             $usertypes=$usertypesdata->value;

		     $uQuery = "select folder_name from #__folder where link_id='$linkId'";
		     $database->setQuery($uQuery);
             $resfolder = $database->loadObject();
             $foldername=$resfolder->folder_name;
         //$foldername=strtolower($link->link_name);
		  ...
		  /* code ends here */
		  $src = $this->config->getjconf('live_site')."/uf_data/".$usertypes."/".$foldername."/logos/".$link->link_image;
			//$src = JURI::root()."media/com_mtree/images/listings/m/".$link->link_image;
			if ($link->link_image ) {

				echo '<div style="width:25%">';
				echo '<a href="' . JRoute::_( 'index.php?option=com_mtree&task=viewlink&link_id=' . $link->link_id . '&Itemid='  . $this->Itemid ) . '"><img src="'.$src.'" alt="'.htmlspecialchars($link->link_name).'" class="image'.(($this->config->getTemParam('imageDirectionListingSummary','left')=='right') ? '':'-left').'" /></a>';
        ...
        $database->setQuery($keywordquery);
         ...
         if(!empty($keywords))
         {
         	echo '<p style="font-size:10px;">'.$keywords[0]->value.'</p>';
         }
