...
class UsersControllerUser extends UsersController
{
	...
	/* new action for changing the company */
	public function multiplecomp()
	{ 
		...
		$userid= $user->id;
		$comp_id=$_REQUEST['comp_ids'];
        $usertypequery = ("SELECT value FROM jos_mt_cfvalues WHERE cf_id = '30' and  link_id='$comp_id'");
        $db->setQuery($usertypequery);
        $usertypesdata = $db->loadObject();
        $usertypes=$usertypesdata->value;
        switch ($usertypes) 
        {
        	case 'Supplier':
        		$usertypes=13;
        		JUserHelper::addUserToGroup($userid,13);
		        JUserHelper::removeUserFromGroup($userid,12);
        		break;
        		case 'Member':
        		$usertypes=12;
        		JUserHelper::addUserToGroup($userid,12);
		        JUserHelper::removeUserFromGroup($userid,13);
        		break;
        	
        	
        }
        $session->set('comp_id',$comp_id);
		$path= JURI::root()."dashboard";
