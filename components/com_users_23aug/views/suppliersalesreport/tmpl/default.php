...
$totals_query = "SELECT mt.link_name,ph.memberid, sum(ph.current_month_value) as current_month_value, 
	sum(ph.previous_yr_current_month_value) as previous_yr_current_month_value,
	sum(ph.yr_to_date_value) as yr_to_date_value, 
	sum(ph.previous_yr_to_date_value) as previous_yr_to_date_value,
	sum(ph.rolling_yr_value) as rolling_yr_value, 
	sum(ph.yoy_ratio_increase) as yoy_ratio_increase
	FROM `jos_purchase_history` as ph, jos_mt_links as mt 
	where ph.supplierid=mt.link_id and mt.link_id='$cmpId' GROUP by ph.memberid  
...
$querySelect = "SELECT  mt.link_name, mt.link_id, mt.alias, ph.* FROM `jos_purchase_history` as ph, jos_mt_links as mt where ph.supplierid='$cmpId' and ph.memberid=mt.link_id and mt.link_published='1' ORDER by mt.link_name";

	$database->setQuery($querySelect);
	$purchistory = $database->loadObjectList();
...
<div id="printableArea">
	...
	<div id="sales_report_history">
		...
		<?php foreach($purchistory as $purchistories){ ?>
			
			<tr>
				<td><?php echo $purchistories->link_name; ?></td>
				<td><?php echo number_format($purchistories->current_month_value, 2); ?></td>
				<td><?php echo number_format($purchistories->previous_yr_current_month_value, 2); ?></td>
				<td><?php echo number_format($purchistories->yr_to_date_value, 2); ?></td>
				<td><?php echo number_format($purchistories->previous_yr_to_date_value, 2); ?></td>
				<td><?php echo number_format($purchistories->rolling_yr_value, 2); ?></td>
				<td><?php echo number_format($purchistories->yoy_ratio_increase,2); ?> %</td>
			</tr>
		<?php } ?>
