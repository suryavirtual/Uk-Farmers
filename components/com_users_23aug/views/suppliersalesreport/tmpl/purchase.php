...
if($usr_id !=''){
	/*$totalQuerySelect = "SELECT sum(ph.current_month_value) as CMV, sum(ph.previous_yr_current_month_value) as MLY, sum(ph.yr_to_date_value) as CYTD, sum(ph.previous_yr_to_date_value) as LYTD, sum(ph.rolling_yr_value) as RYV FROM `jos_purchase_history` as ph, jos_mt_links as mt where ph.memberid='204' and ph.supplierid=mt.link_id and mt.link_published='1' ORDER by mt.link_name";
	...
	<table width="100%" class="supplier-history">
		...
		<?php foreach($purchaseDtl as $purchaseDtls){
			...
			$cmpUrl = JURI::root()."supplier-search/".$cmpId."-".$cmpAlias;?>
		<tr>
			<td align="right"><a href="<?php echo $cmpUrl; ?>"><?php echo $purchaseDtls->link_name; ?></a></td>
