...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	//$getQuerySelect .= " where mt.link_id='".$supplier_id ."'";
	...
	$getQuerySelect .= " where sf.comp_id='".$supplier_id ."'";

	$getQuerySelect .= " and  sf.type=dt.id and sf.expiry >= date_sub(now(), interval 6 month)  ";
	...
	if(!empty($val)){
	 ...
	 $getQuerySelect .= " order by sf.description ";
	...
	while ($sendFile = mysql_fetch_object($querylimmit)){
		...
		<tr>
			...
			<td style ="<?php echo $cls; ?>"><?php echo $expiry_date ;?></td>
					...
					<div id="inline_files<?php echo $k; ?>" style='padding:10px; background:#fff; height:400px; overflow-x:scroll;'>
						...
						<table width='100%' border="1">
							...
							while($attachedFiles = mysql_fetch_object($attachedFile)){
								...
								echo "<tr><td>".$f."</td>
								<td><a href='https://".$_SERVER['SERVER_NAME']."/".$sendFile->filename."' target='_blank'>".$attachedFiles->fname."</a></td>
								</tr>";
