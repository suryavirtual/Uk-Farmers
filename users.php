...
{ 
       ...
       // $selAllUsers = mysql_query("SELECT u.*, ug.group_id as grpId FROM `jos_users` as u,  jos_user_usergroup_map as ug where u.comp_list in(".$memberid.") and u.id=ug.user_id order by name");
        
      $selAllUsers = mysql_query("SELECT u.* ,umap.group_id as grpId FROM jos_users as u , jos_user_usergroup_map as umap where u.id=umap.user_id and (FIND_IN_SET(".$memberid.",u.comp_list))
       and umap.group_id not IN('7','8')group by umap.user_id order by name");
 
	/*  $selAllUsers = mysql_query("SELECT u.* ,GROUP_CONCAT(DISTINCT umap.group_id SEPARATOR ',')as grpId FROM jos_users as u , jos_user_usergroup_map as Umap where u.id=umap.user_id and 
         (FIND_IN_SET(".$memberid.",U.comp_list)) and umap.group_id not IN('7','8') group by umap.user_id order by u.name "); */
			...
			while ($getAllRes = mysql_fetch_object($selAllUsers))
			{
				echo '<tr>';
					...
					while ($getAllDoc = mysql_fetch_object($selAllDoc))
					{
					 ...
					 // $groupArr = array();
                                         // $groupArr=explode(",", $getAllRes->grpId);	
...
$tableClose = '</table>';
           ...
           // $selUsers = mysql_query("SELECT u.*, ug.group_id as grpId FROM `jos_users` as u,  jos_user_usergroup_map as ug where u.comp_list in(".$memId.") and u.id=ug.user_id order by u.name");
           $selUsers = mysql_query("SELECT u.* ,umap.group_id as grpId FROM jos_users  as u , jos_user_usergroup_map  as umap where u.id=umap.user_id and
	 (FIND_IN_SET(".$memId.",u.comp_list)) and umap.group_id not IN('7','8') group by umap.user_id order by u.name"); 

       /*  $selUsers = mysql_query("SELECT u.* ,GROUP_CONCAT(DISTINCT umap.group_id SEPARATOR ',')as grpId FROM jos_users as u , jos_user_usergroup_map as Umap where u.id=umap.user_id and
        (FIND_IN_SET(".$memId.",U.comp_list)) and umap.group_id not IN('7','8') group by umap.user_id order by u.name "); */
...
if($memId == 'all'){
	...
	//  $selUsers = mysql_query("SELECT u.*, ug.group_id as grpId FROM `jos_users` as u,  jos_user_usergroup_map as ug where u.comp_list in(".$memId.") and u.id=ug.user_id order by u.name");
        $selUsers = mysql_query("SELECT u.* ,umap.group_id as grpId FROM jos_users  as u , jos_user_usergroup_map  as umap where u.id=umap.user_id and (FIND_IN_SET(".$memId.",u.comp_list))
        and umap.group_id not IN('7','8')group by umap.user_id order by u.name");
 
    /*  echo "@query".$testquery="SELECT u.* ,GROUP_CONCAT(DISTINCT umap.group_id SEPARATOR ',')as grpId FROM `jos_users` as `u` , `jos_user_usergroup_map` as `Umap`where u.id=umap.user_id
         and (FIND_IN_SET(".$memId.",U.comp_list)) and umap.group_id not IN('7','8') group by umap.user_id order by u.name ";
         $selUsers=mysql_query($testquery);
        while ($getRes = mysql_fetch_object($selUsers)){echo "<pre>";print_r($getRes);}
         $selUsers = mysql_query("SELECT u.* ,GROUP_CONCAT(DISTINCT umap.group_id SEPARATOR ',')as grpId FROM jos_users as u , jos_user_usergroup_map as Umap where u.id=umap.user_id and (FIND_IN_SET(".$memId.",U.comp_list))
        and umap.group_id not IN('7','8') group by umap.user_id order by u.name "); */ 
}
...
{ 
	while($allMem = mysql_fetch_object($queryMember))
	{
		...
		echo '<tbody>';
		//$selAllUsers = mysql_query("SELECT u.*, ug.group_id as grpId FROM `jos_users` as u,  jos_user_usergroup_map as ug where u.comp_list in(".$allMem->link_id.") and u.id=ug.user_id order by name");
                  $selAllUsers = mysql_query("SELECT u.* ,umap.group_id as grpId FROM jos_users as u, jos_user_usergroup_map  as umap where u.id=umap.user_id and 
			(FIND_IN_SET(".$allMem->link_id.",u.comp_list)) and umap.group_id not IN('7','8')group by umap.user_id order by name"); 
                  
               /*  $selAllUsers  = mysql_query("SELECT u.* ,GROUP_CONCAT(DISTINCT umap.group_id SEPARATOR ',')as grpId FROM jos_users  as u , jos_user_usergroup_map  as Umap where u.id=umap.user_id and
                    (FIND_IN_SET(".$allMem->link_id.",U.comp_list)) and umap.group_id not IN('7','8') group by umap.user_id order by u.name "); */  
