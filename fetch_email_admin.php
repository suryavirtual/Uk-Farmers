...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	$getdata1= mysql_query("select user_id from jos_mt_links where link_id='$supplier_id'");

	while ($sendFiledata = mysql_fetch_object($getdata1))

	{

      $user_id=$sendFiledata->user_id;
	...
	<table class="terms-admin ">

		<tr>

			<th class="one">Sr No</th>

			<th class="tab22">Description</th>
			<th class="tab22">Sent To</th>

	<th class="tab44">Category</th>

			<!--<th style="width: 19.2%;">Sent To</th>-->

			<!--<th>Expiry</th>-->

			<th class="tab55">Share</th>
	...
	$getQuerySelect = "SELECT  em.*,dt.doc_name  FROM  jos_email_member as em,jos_document_type as dt ";

	$getQuerySelect .= " where em.supplierCmpId='".$supplier_id ."' and em.document_type=dt.id";

	//$getQuerySelect .= " and mt.link_id=u.comp_list";

	

	if(!empty($val)){

		$getQuerySelect .= "and em.subject like '%$val%' ";

	}
	$getQuerySelect .= " order by em.subject ";

//echo "query------->".$getQuerySelect;
	...
	while ($sendFile = mysql_fetch_object($querylimmit)){
		...
		$shareMember = mysql_query("select em.*, mt.link_name from jos_send_email_member as em, jos_mt_links as mt where em.emId='$sendFile->id' and em.memberId=mt.link_id and mt.link_published='1' group by mt.link_name");
		...
		<tr>
			...
			<td><?php echo $sendFile->subject ;?></td>
			...
			<!--<td class="<?php echo $cls; ?>"><?php echo $expiry_date ;?></td>-->
					...
					<div id="inline_files<?php echo $k; ?>" style='padding:10px; background:#fff; height:400px; overflow-x:scroll;'>
						...
						<table width='100%' border="1">
							...
							while($attachedFiles = mysql_fetch_object($attachedFile)){
								...
								echo "<tr><td>".$f."</td>
								<td><a href='http://".$_SERVER['SERVER_NAME']."/uk_farmers/".$sendFile->filename."' target='_blank'>".$attachedFiles->fname."</a></td>
								</tr>";
