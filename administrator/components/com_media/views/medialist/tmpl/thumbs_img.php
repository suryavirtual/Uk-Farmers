...
<li class="imgOutline thumbnail height-80 width-80 center">
	...
	<div class="height-50">
		<a class="img-preview" href="<?php echo COM_MEDIA_BASEURL . '/' . $this->_tmp_img->path_relative; ?>" title="<?php echo $this->_tmp_img->name; ?>" >
			<?php
	$filextension=$this->_tmp_img->filetype;
			if($filextension== "pdf"){  ?>
			...
			<?php } else if($filextension == 'doc'){?>
			<img src="<?php echo JURI::root() . 'images/doc.png' ?>" alt="<?php echo $this->_tmp_img->title; ?>" width="<?php //echo $this->_tmp_img->width_60; ?>" height="<?php //echo $this->_tmp_img->height_60; ?>" />
			<?php } else if($filextension== 'xlsx'){?>
			...
			<?php } else if($filextension == 'xls'){?>
				...
				<?php } else {
				echo JHtml::_('image', COM_MEDIA_BASEURL . '/' . $this->_tmp_img->path_relative, JText::sprintf('COM_MEDIA_IMAGE_TITLE', $this->_tmp_img->title, JHtml::_('number.bytes', $this->_tmp_img->size)), array('width' => $this->_tmp_img->width_60, 'height' => $this->_tmp_img->height_60));
           }
				?>
		</a>
	</div>
