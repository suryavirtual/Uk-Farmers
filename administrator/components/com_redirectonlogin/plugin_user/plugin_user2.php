...
class plgUserRedirectonlogin extends JPlugin{	
	...
	function onUserLogin($user, $options){	
		...
		$database->setQuery("SELECT comp_list "
		."FROM #__users  "
		."WHERE email='$user_email' "
		."LIMIT 1 "
		);
		$rows = $database->loadObject();
		$comp_ids=explode(',',$rows->comp_list);
		...
		if($total_comp == "1")
			{
				//die("one company user");
			  $redirect_url = $helper->get_link_from_menuitem($this->rol_config['menuitem_login']);	
			}
			else
			{
				
				//die("Multiple company user");
				
				$redirect_url  = '/index.php?option=com_users&view=compcheck';
