...
<script>
function fetchingemails(supplier_ids)
{
	var supplier_ids=supplier_ids
   jQuery("#results" ).load("<?php echo JURI::root();?>/fetch_email_admin.php",{user_id: supplier_ids}); 	
}
jQuery(function() {
	//alert("this is just testings");
	jQuery("#add_files").tabs();
	jQuery("#supplier").val(jQuery("#supplierCmp option:selected").text());
	jQuery("#suppliers").val(jQuery("#supplierCmp option:selected").val());
...
$q2="select comp_list from #__users where id='".$fileDtl->supplierid."'";
$database->setQuery( $q2 );
$supplierid = $database->loadObject();
 $getsupplierid=$supplierid->comp_list;
...
$database->setQuery( $q1 );
$fileDtl = $database->loadObject();
$newsupplierid=$fileDtl->supplierCmpId;
?>
<script>
jQuery(function() 
{
fetchingemails('<?php echo $newsupplierid; ?>'); 
});
</script>
<div id="add_files">
<ul>
			...
			<fieldset class="adminform">
				...
				<div class="controls">
					...
					<?php if(!empty($fileDtl->file)){ ?>
						<a href="<?php echo JURI::root() ?>uf_data/upload/email/<?php echo $fileDtl->file; ?>" target="_blank">Download</a>
					<?php }?>
...
<div id="supplier_email_member">
	<form action="index.php?option=com_smadmin&task=emails.sendemails" method="post" id="adminForm" name="adminForm">
	<div class="control-label select-left">Supplier Name</div>
		<div class="controls">

      <input type="text" name="suppliers" id="supplier" class="select-right" readonly="true" />
      <input type="hidden" name="supplier" id="suppliers" class="select-right" />
		...
		<div id="results">
			...
			<tr>
			<td><input type="submit" class="email_mem_submit btn-success" name="send" value="Send Email"></td>
			</tr>
