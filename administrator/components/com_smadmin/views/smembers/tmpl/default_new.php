...
<select name="supplier" id="supplier_name">
        ...
        if(data)
        {
            console.log(data);    
               ...
               jQuery.each(parsedJson, function (i,k) 
              {
                ...
                if(i !="doc" && k.length>0)
                {
              row1='<tr><td><strong>Member:'+i+'<strong></td><td></td><td></td><td></td><td></td><td></td></tr>';
...
function datapass(company_ids,supplier_id)
{
  var sitePath = '<?php  echo JURI::root() ?>';
    ...
    var url = sitePath + 'administrator/index.php?option=com_smadmin&task=Smembers.getusers';
        ...
        type: 'post',
        url: url,
        data: {company_id:company_ids,supplier_ids:supplier_id},
        success:function(data)
        {
        if(data)
        {
            console.log(data);    
                ...
                jQuery.each(parsedJson.doc, function (k,m) 
              {
                ...
                row+='type="checkbox" name="document['+v.user_id+'][]" value="'+m.id+'">';
...
jQuery("#company_name").on("change", function()
    {
     ...
     alldata();
        ...
        { 
          ...
          {
            datapass(company_ids,supplier_id);
          }
