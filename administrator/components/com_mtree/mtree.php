...
function savelink( $option, $post=null ) {
 ...
 {
	$db = JFactory::getDbo();
    $foldername=strtolower($post['link_name']);
	$companytype=$post['cf30'];
	if($companytype=="Member")
	{
       $types="members";
	}
			...
			if ( $new_link )
			 {
				...
				$newlinkids = $database->loadResult();
				  	/*create new folder at the time of creation for company*/
     if($companytype=="Member")
	{
    $feedDir = $_SERVER['DOCUMENT_ROOT']."/uf_data/members/";
      if(!is_dir($feedDir."/".$foldername))
                 {
                 mkdir($feedDir."/".$foldername, 0777, true);
                 mkdir($feedDir."/".$foldername."/logos", 0777, true);
                 }

	}
	if($companytype=="Supplier")
	{
               $feedDir = $_SERVER['DOCUMENT_ROOT']."/uf_data/suppliers/";
                if(!is_dir($feedDir."/".$foldername))
                 {
                 mkdir($feedDir."/".$foldername, 0777, true);
                 mkdir($feedDir."/".$foldername."/file_links", 0777, true);
                 mkdir($feedDir."/".$foldername."/terms", 0777, true);
                 mkdir($feedDir."/".$foldername."/marketing", 0777, true);
                 mkdir($feedDir."/".$foldername."/pricelists", 0777, true);
                 mkdir($feedDir."/".$foldername."/offers", 0777, true);
                 mkdir($feedDir."/".$foldername."/logos", 0777, true);
                 }
	} 

		
        	/*end here for creating folder */

/*insert folder name into database */
        $query = "insert into #__folder values ('','$newlinkids','$foldername')";
        $db->setQuery($query);
		$db->query();
/* code ends here */
			...
			{
			 $linkids=$post['link_id'];
	
		    $db = JFactory::getDbo();	
/* code for updating the value for old folder name */
  if($companytype=="Member")
	{
		$types="members";
	}
	if($companytype=="Supplier")
	{
        ...
        $uQuery = "select * from #__folder where link_id='$linkids'";
		$db->setQuery($uQuery);
        $resfolder = $db->loadObject();
        $oldfolder=$resfolder->folder_name;
        

        if(!empty($resfolder))
        {
		 $old_foldername= $_SERVER['DOCUMENT_ROOT']."/uf_data/".$types."/".$oldfolder;
         $new_foldername=$_SERVER['DOCUMENT_ROOT']."/uf_data/".$types."/".$foldername;
         rename($old_foldername,$new_foldername);
        	$query = "update #__folder set folder_name='$foldername' where link_id='$linkids' ";
			 $db->setQuery($query);
		     $db->query();
		     /*query for fetching terms files and update files path in terms table */
        $termquery = "select termId,termsFile from #__structured_terms  where supplierUserId='$linkids'";
		$db->setQuery($termquery);
        $resterms = $db->loadObjectList();
        foreach ($resterms as $value) 
        {
        	$termid=$value->termId;
        	$termpath=$value->termsFile;
        	$arr = explode("/",$termpath);
        	$arr[2] = $foldername;
        	$newterm = implode("/", $arr);
        	$tquery = "update #__structured_terms set termsFile='$newterm' where termId='$termid' ";
			$db->setQuery($tquery);
		    $db->query();
       ...
       /*query ends here */
       /*query for fetching  files and update files path in files table */
        $filequery = "select id,filename from #__supplier_files  where comp_id='$linkids'";
		$db->setQuery($filequery);
        $resfiles = $db->loadObjectList();
        foreach ($resfiles as $value) 
        {
        	...
        	$arr[2] = $foldername;
        	$newfile = implode("/", $arr);
            $fquery = "update #__supplier_files set filename='$newfile' where id='$fileid' ";
			$db->setQuery($fquery);
		    $db->query();
       ...
       /*query ends here for file path update in files table */
       /*query start here for fetching tabs data */
       $tabquery = "select id,description from #__company_tab  where comp_id='$linkids'";
		$db->setQuery($tabquery);
        $restabs = $db->loadObjectList();

        foreach($restabs as $tabval)
        {
            $tabid=$tabval->id;
        	$desc = $tabval->description;
        	$del = 'href';
        	$arra = explode($del, $desc);
        	...
        	foreach($arra as $val) 
        	{
        		if($val[0]=="=")
        		{
        			...
        			$del2 = "/";
        			$arra2 = explode($del2, $val);
        			if($foldername != $arra2[4] && $arra2[2]=='uf_data')
        			{
        				$arra2[4] = $foldername;	
        			}
        			$stringifyarra2 = implode("$del2", $arra2);

        			$arra[$count] =  $stringifyarra2;
        		}
        		$count++;
        	}
        	$descnew = implode($del, $arra);
        	$tabupdatequery = "update #__company_tab set description='$descnew' where id='$tabid' ";
			$db->setQuery($tabupdatequery);
		    $db->query();
       ...
       /* query ends here */
        ...
        {
        $query = "insert into #__folder values ('','$linkids','$foldername')";
        $db->setQuery($query);
		$db->query();
        }
		...
		if( !is_writable($mtconf->getjconf('absolute_path').$mtconf->get('relative_path_to_listing_small_image')) || !is_writable($mtconf->getjconf('absolute_path').$mtconf->get('relative_path_to_listing_medium_image')) ||  !is_writable($mtconf->getjconf('absolute_path').$mtconf->get('relative_path_to_listing_original_image')) ) {
			...
			for($i=0;$i<count($files['image']['name']);$i++) {
				if ( !empty($files['image']['name'][$i]) && $files['image']['error'][$i] == 0 &&  $files['image']['size'][$i] > 0 ) {
					...
					//custom code for saving in logo folder start from here 
                   $linkids=$row->link_id;
                  ...
                  $resfolder = $db->loadObject();
                   $foldername=$resfolder->folder_name;
					...
					move_uploaded_file($files['image']['tmp_name'][$i], $_SERVER['DOCUMENT_ROOT']."/uf_data/".$usertypes."/".$foldername."/logos/".$files['image']['name'][$i] );
					...
					rename($_SERVER['DOCUMENT_ROOT']."/uf_data/".$usertypes."/".$foldername."/logos/".$files['image']['name'][$i], $_SERVER['DOCUMENT_ROOT']."/uf_data/".$usertypes."/".$foldername."/logos/".$img_id.'.'.$file_extension);
