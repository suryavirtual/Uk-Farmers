...
class mFieldType_coredesc extends mFieldType {
	...
	function getOutput($view=1) {
		...
		$params['allowedTags'] = $this->getParam('allowedTags','u,b,i,a,ul,li,pre,blockquote,strong,em,table,td,tr,img,p');
		...
		// Summary view
			...
			if (JString::strlen($html) > $params['summaryChars']) {
				$output .= $this->truncate($trimmed_desc,300);
				$output .= ' <b>...</b>';
			} else {
				$output = $html;
			}
		...
		/* new function added for triming the word */
