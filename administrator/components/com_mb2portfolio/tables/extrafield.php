 ...
 */
class Mb2portfolioTableextrafield extends JTable {
    ...
    public function __construct(&$db) {
        parent::__construct('#__mb2portfolio_extra_fields', 'id', $db);
    }
    ...
    public function bind($array, $ignore = '') {
		...
		if(($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.edit.state','com_mb2portfolio') && $array['state'] == 1)){
			$array['state'] = 0;
		}

		if ( isset($array['tags']) && !empty($array['tags']) ) {
			// Load the tags helper.
			require_once JPATH_ADMINISTRATOR.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_tags'.DIRECTORY_SEPARATOR.'helpers'.DIRECTORY_SEPARATOR.'tags.php';

			// Get the allowed actions for the user
			$canDo = TagsHelper::getActions(); // The helper get the user and the component name itself

			// Load the tags model.
			require_once JPATH_ADMINISTRATOR.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_tags'.DIRECTORY_SEPARATOR.'models'.DIRECTORY_SEPARATOR.'tag.php';
			JTable::addIncludePath(JPATH_ADMINISTRATOR.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_tags'.DIRECTORY_SEPARATOR.'tables');
	...
	public function generateAlias()
	{
		...
		$db = JFactory::getDBO();
		$db->setQuery("SELECT id FROM #__mb2portfolio_extra_fields WHERE alias = ".$db->quote($this->alias)." AND id != ".(int)$this->id);
    ...
    */
    protected function _getAssetName() {
        $k = $this->_tbl_key;
        return 'com_mb2portfolio.extrafield.' . (int) $this->$k;
    }
