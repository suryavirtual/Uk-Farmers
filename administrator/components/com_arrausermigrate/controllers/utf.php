...
class ArrausermigrateControllerUtf extends ArrausermigrateController{
    ...
    function utf(){
		JFactory::getApplication()->input->set("view", "utf");
	...
	function import(){
		...
		if(isset($message_array)){		    
			if(isset($message_array[0]) && $message_array[0]=="ERROR"){
				$this->setRedirect(JURI::base()."index.php?option=com_arrausermigrate&controller=utf", $message_array[1], 'notice');
			}
			else{
				$this->setRedirect(JURI::base()."index.php?option=com_arrausermigrate&controller=utf", $message_array[1]);
			}   
		}
