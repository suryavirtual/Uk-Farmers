...
class ArrausermigrateViewExport extends JViewLegacy{
	...
	function display($tpl = null){						
		...
		JToolBarHelper::custom('export_button', 'minus-sign.png', 'minus-sign.png', 'Export', false, false);
	...
	function secondColumnExport1(){
	    $columns_value = array("id", "password", "usertype", "block");
		$columns_name = array("User ID", "Password", "User Type", "Blocked");
	...
	function userProfileFields(){
		...
		if(isset($user_profile_fields) && is_array($user_profile_fields) && count($user_profile_fields)>0){
			foreach($user_profile_fields as $key=>$value){
				if($value["profile_key"] != "profile.profile"){
				...
				elseif($type == "radio"){
			   	...
			   	$columns .= "<tr>";
					if (class_exists('ZipArchive')){				
						$columns .= "<td class=\"td-radio-checkbox\">";  
	...
	function generateCheckboxProfile($type, $class, $header_column_class, $td_class, $element_name, $columns_value, $columns_name, $checked="", $all_user_type=""){		
		...
		$columns .= 		'<span style="float:left; padding-right:10px;">'.JText::_("ARRA_CHECK_ALL_FIELDS")."</span>";
		$columns .= '		<input style="float:left;" type="checkbox" name="toggle" id="toggle" onclick="javascript:checkAllFields(\''.implode(",", $collumns_array).'\', \'adminForm\');" />';		
		$columns .= '	</td>';
		$columns .= '</tr>';
	...
	function setOrdering(){
		...
		$combo .=     "</td>";
		$combo .= 	   "<td>";
		$combo .= 			'<fieldset class="radio btn-group" id="mode_order">';
        $combo .= 				'<input type="radio" value="asc" name="mode_order" id="jform_mode_order0" checked="checked">';
        $combo .= 				'<label for="jform_mode_order0" class="btn">'.JText::_("ARRA_ORDER_ASC").'</label>';
        $combo .= 				'<input type="radio" value="desc" name="mode_order" id="jform_mode_order1">';
        $combo .= 				'<label for="jform_mode_order1" class="btn">'.JText::_("ARRA_ORDER_DESC").'</label>';
		$combo .= 			'</fieldset>';
		$combo .= 	   "</td>";
		$combo .= 	"</tr>";
		$combo .= "</table>";
	...
	function setEmailTo(){
		...
		$email_to .= 		"<input type=\"checkbox\" name=\"email_to_super_admin\" style=\"float:left; margin-right:10px;\">";
		$email_to .= 		"<div style=\"float:left;\" class=\"editlinktip hasTip\" title=\"" . JText::_("ARRA_SUPER_ADMIN_EMAIL_CHECK")."::".JText::_("ARRA_TOOLTIP_SUPER_ADMIN_EMAIL_CHECK"). "\" >";
		$email_to .= 			JText::_("ARRA_SUPER_ADMIN_EMAIL_CHECK");
		$email_to .= 		"</div>";
		$email_to .= 	"</td>";			
		$email_to .= "</tr>";
		$email_to .= "<tr>";
