...
// Options button.
if (JFactory::getUser()->authorise('core.admin', 'com_arrausermigrate')) {
	JToolBarHelper::preferences('com_arrausermigrate');
}
...
class ArrausermigrateViewArrausermigrate extends JViewLegacy{
	...
	function countUsers($usertype){
		...
		$db = JFactory::getDBO();
		$sql = "select count(*) from #__user_usergroup_map where group_id in (select id from #__usergroups where title='".addslashes(trim($usertype))."')";				
		$db->setQuery($sql);
	...
	function getPicture($value){	
	    $picture = "";
		$picture .= '<div class="progress progress-striped active">
