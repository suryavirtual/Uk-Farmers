...
class ArrausermigrateModelEditfield extends JModelLegacy{
	...
	function saveFields(){	
		...
		$name = trim(JFactory::getApplication()->input->get("name", "", "raw"));
		$type = trim(JFactory::getApplication()->input->get("field_type", "text", "raw"));
		...
		if(isset($field) && isset($field["1"]["0"])){
			...
			$field_type = JFactory::getApplication()->input->get("field_type", "", "raw");
			$field_id = JFactory::getApplication()->input->get("field_id", "", "raw");
			$field_description = JFactory::getApplication()->input->get("field_description", "", "raw");
			$field_filter = JFactory::getApplication()->input->get("field_filter", "", "raw");
			$field_label = JFactory::getApplication()->input->get("field_label", "", "raw");
			$field_message = JFactory::getApplication()->input->get("field_message", "", "raw");
			$field_size = JFactory::getApplication()->input->get("field_size", "", "raw");
			$field_cols = JFactory::getApplication()->input->get("field_cols", "", "raw");
			$field_rows = JFactory::getApplication()->input->get("field_rows", "", "raw");
			$field_options = JFactory::getApplication()->input->get("field_options", "", "raw");
			...
			if($type == "radio" || $type == "list" || $type == "checkboxes"){
				$new_values .= "\t"."\t"."\t".">"."\n";
				$set[] = "`option`='".$db->escape($field_options)."'";
				$options = explode("\n", trim($field_options));
				if(isset($options) && count($options) > 0){
					foreach($options as $key=>$value){
						if(trim($value) != ""){
							$temp = explode("-", trim($value));
							$new_values .= "\t"."\t"."\t"."\t".'<option value="'.trim(str_replace('&', '&amp;', $temp["0"])).'">'.trim(str_replace('&', '&amp;', $temp["1"])).'</option>'."\n";
						}
					}					
				}
			}
