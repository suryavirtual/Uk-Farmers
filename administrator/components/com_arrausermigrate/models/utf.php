...
require_once (JPATH_COMPONENT_ADMINISTRATOR.DIRECTORY_SEPARATOR.'helpers'.DIRECTORY_SEPARATOR.'TxtImport.php');
...
class ArrausermigrateModelUtf extends JModelLegacy{
	...
	function getExport(){
		...
		if($task != ""){
			$userType = array();
			$column = array();
			$top_column_checkbox = JFactory::getApplication()->input->get("top_column_checkbox", "", "raw");
			
			if(isset($top_column_checkbox)){
				$user_type_checkbox = JFactory::getApplication()->input->get("user_type_checkbox", "EMPTY", "raw");
				
				if(is_array($user_type_checkbox) && !in_array("EMPTY", $user_type_checkbox)){
					$userType = JFactory::getApplication()->input->get("user_type_checkbox", "", "raw");
				...
				$column = JFactory::getApplication()->input->get("top_column_checkbox", array(), "raw");
				$order = JFactory::getApplication()->input->get("ordering", "0", "raw");
				$mode_order = JFactory::getApplication()->input->get("mode_order", "0", "raw");
	...
	function csvExport($userType, $column, $order="", $mode_order=""){	  	   
		...
		$split_name = JFactory::getApplication()->input->get("split_name", "no", "raw");
		...
		$remove_header = JFactory::getApplication()->input->get("remove_header", "", "raw");
	...
	function sendMail($data, $file_name, $list_emails){
			...
			if(!is_dir(JPATH_SITE .DIRECTORY_SEPARATOR."administrator".DIRECTORY_SEPARATOR."components".DIRECTORY_SEPARATOR."com_arrausermigrate".DIRECTORY_SEPARATOR."files")){
				mkdir(JPATH_SITE .DIRECTORY_SEPARATOR."administrator".DIRECTORY_SEPARATOR."components".DIRECTORY_SEPARATOR."com_arrausermigrate".DIRECTORY_SEPARATOR."files", 0777); 
			}
			//if file name exist, then set pathe for write in this file		 
			if(isset($file_name)){		
				$file_path = JPATH_SITE .DIRECTORY_SEPARATOR."administrator".DIRECTORY_SEPARATOR."components".DIRECTORY_SEPARATOR."com_arrausermigrate".DIRECTORY_SEPARATOR."files".DIRECTORY_SEPARATOR. $file_name;
			}
	...
	function import(){		
		...
		$separator = JFactory::getApplication()->input->get("separator", ",", "raw");
		$overwrite_usertype = JFactory::getApplication()->input->get("same_user_option_radio_usertype", "", "raw");
		$overwrite_password = JFactory::getApplication()->input->get("same_user_option_radio_password", "", "raw");
		$overwrite_email = JFactory::getApplication()->input->get("same_user_option_radio_email", "", "raw");
		$overwrite_block = JFactory::getApplication()->input->get("same_user_option_radio_block", "", "raw");
		$overwrite_params = JFactory::getApplication()->input->get("same_user_option_radio_params", "", "raw");
		$password_encripted = JFactory::getApplication()->input->get("encripted_password_radio", "", "raw");
		$generate_password = JFactory::getApplication()->input->get("generate_password_radio", "", "raw");
		...
		$data = JFactory::getApplication()->input->get("file_content", "", "raw");
	...
	function csv_txtImport($data){
		...
		if(isset($rows) && is_array($rows) && count($rows)>0){					
			while(isset($rows[$iterator]) && trim($rows[$iterator]) != ""){				   	    
				...
				if($i == "header"){	
					...
					$default_password = JFactory::getApplication()->input->get("default_password", "", "raw");
					$generate_password = JFactory::getApplication()->input->get("generate_password_radio", "1", "raw");
					
					if(!in_array("password", $columns_imported) && strlen(trim($default_password)) == 0 && $generate_password==0){
				...
				//else if is a row(from body content)
					...
					if($user_id_from_database == -1){				
						...
						if($this->emailExist($email) == false){
								...
								if(count($user_groups) > 0){
									foreach($user_groups as $group_key=>$group_value){
										if(!in_array(trim($group_value), $all_user_type)){
											...
											if(trim($group_value) == ""){
												$usertype = JFactory::getApplication()->input->get("position", "Registered", "raw");
							...
							$change_admin_users = JFactory::getApplication()->input->get("super_admin_users", "no", "raw");
		...
		if(count($users_existent)>0){
			...
			$registry->set('link_eror', "error");
			
			$file_path = JPATH_SITE .DIRECTORY_SEPARATOR."administrator".DIRECTORY_SEPARATOR."components".DIRECTORY_SEPARATOR."com_arrausermigrate".DIRECTORY_SEPARATOR."files".DIRECTORY_SEPARATOR."error_same_email.csv";
			$g = fopen ($file_path, "w");
		...
		if(count($empty_columns)>0){
			$session = JFactory::getSession();
			$registry = $session->get('registry');
			$registry->set('error_empty_column', "error_empty_column");
			
			$file_path = JPATH_SITE .DIRECTORY_SEPARATOR."administrator".DIRECTORY_SEPARATOR."components".DIRECTORY_SEPARATOR."com_arrausermigrate".DIRECTORY_SEPARATOR."files".DIRECTORY_SEPARATOR."error_empty_column.csv";
			$g = fopen ($file_path, "w");
			$data = implode($separator, $columns_imported)."\n";
