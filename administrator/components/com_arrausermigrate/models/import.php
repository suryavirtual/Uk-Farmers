...
require_once (JPATH_ADMINISTRATOR.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_arrausermigrate'.DIRECTORY_SEPARATOR.'helpers'.DIRECTORY_SEPARATOR.'TxtImport.php');
require_once (JPATH_ADMINISTRATOR.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_arrausermigrate'.DIRECTORY_SEPARATOR.'helpers'.DIRECTORY_SEPARATOR.'SqlExport.php');
...
class ArrausermigrateModelImport extends JModelLegacy{
	...
	function import(){
		...
		$file = JFactory::getApplication()->input->files->get( 'sqlzip_file_upload', NULL);
		
		if($file['name'] != ""){
			$file = JFactory::getApplication()->input->files->get( 'sqlzip_file_upload', NULL);
		}
		else{
			$file = JFactory::getApplication()->input->files->get( 'csvtxt_file_upload', NULL);
		}
			...
			//else if is a row(from body content)
				...
				if($user_id_from_database == -1){				
					...
					if($this->emailExist($email) === false){
						...
						$change_admin_users = JFactory::getApplication()->input->get("super_admin_users", "0", "raw");
						
						if($change_admin_users == "1"){
							if(in_array("usertype", $columns_imported)){
								$temp = array_keys($columns_imported, "usertype");
								$position = $temp["0"];
	...
	function emailExist($email){
		$validate_email = JFactory::getApplication()->input->get("validate_email", "0", "raw");
	...
	function backUp(){
		...
		$path = JPATH_SITE .DIRECTORY_SEPARATOR."administrator".DIRECTORY_SEPARATOR."components".DIRECTORY_SEPARATOR."com_arrausermigrate".DIRECTORY_SEPARATOR."files".DIRECTORY_SEPARATOR. $sql_file_name;	
		//if file not exist then create a new file
		if(!is_file($path)){	  
			$this->mkfile(JPATH_SITE .DIRECTORY_SEPARATOR."administrator".DIRECTORY_SEPARATOR."components".DIRECTORY_SEPARATOR."com_arrausermigrate".DIRECTORY_SEPARATOR."files".DIRECTORY_SEPARATOR. $sql_file_name, 0777);
		}		
		$zip = new ZipArchive;
