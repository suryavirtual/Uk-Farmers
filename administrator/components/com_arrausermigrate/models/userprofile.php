...
class ArrausermigrateModelUserprofile extends JModelLegacy{
	...
	function recreatefields(){
		...
		$file_profile_path = JPATH_SITE.DIRECTORY_SEPARATOR."plugins".DIRECTORY_SEPARATOR."user".DIRECTORY_SEPARATOR."profile".DIRECTORY_SEPARATOR."profiles".DIRECTORY_SEPARATOR."profile.xml";
		$file_content = JFile::read($file_profile_path);
		
		if(isset($fields) && is_array($fields) && count($fields) > 0){
			foreach($fields as $key=>$field){
				if(strpos($file_content, 'id="'.$field["field_id"].'"') === FALSE){
					JFactory::getApplication()->input->set("number_columns", "1");
					JFactory::getApplication()->input->set("recreate", "1");
					JFactory::getApplication()->input->set("field_name_1", $field["name"]);
					JFactory::getApplication()->input->set("field_type_1", $field["type"]);
					JFactory::getApplication()->input->set("field_id_1", $field["field_id"]);
					JFactory::getApplication()->input->set("field_description_1", $field["description"]);
					JFactory::getApplication()->input->set("field_filter_1", $field["filter"]);
					JFactory::getApplication()->input->set("field_label_1", $field["label"]);
					JFactory::getApplication()->input->set("field_message_1", $field["message"]);
					JFactory::getApplication()->input->set("field_cols_1", $field["cols"]);
					JFactory::getApplication()->input->set("field_rows_1", $field["rows"]);
					JFactory::getApplication()->input->set("field_option_1", $field["option"]);
					JFactory::getApplication()->input->set("field_size_1", $field["size"]);
	...
	function saveFields(){		
		$db = JFactory::getDBO();
		$file_profile_path = JPATH_SITE.DIRECTORY_SEPARATOR."plugins".DIRECTORY_SEPARATOR."user".DIRECTORY_SEPARATOR."profile".DIRECTORY_SEPARATOR."profiles".DIRECTORY_SEPARATOR."profile.xml";
		$file_profile_plugin = JPATH_SITE.DIRECTORY_SEPARATOR."plugins".DIRECTORY_SEPARATOR."user".DIRECTORY_SEPARATOR."profile".DIRECTORY_SEPARATOR."profile.xml";
		$file_user_profile_content = JFile::read($file_profile_path);
		$file_profile_plugin_content = JFile::read($file_profile_plugin);		
		$rows_number = JFactory::getApplication()->input->get("number_columns", "0", "raw");
		
		JFactory::getApplication()->input->set("recreate", "0");
		...
		if($rows_number != "0"){
			for($i=1; $i<=$rows_number; $i++){
				$name = strtolower(JFactory::getApplication()->input->get("field_name_".$i, "", "raw"));
				$name = str_replace(" ", "", $name);
				$type = JFactory::getApplication()->input->get("field_type_".$i, "", "raw");
				$id = strtolower(JFactory::getApplication()->input->get("field_id_".$i, "", "raw"));
				...
				$description = JFactory::getApplication()->input->get("field_description_".$i, "", "raw");
				$filter = JFactory::getApplication()->input->get("field_filter_".$i, "", "raw");
				$label = JFactory::getApplication()->input->get("field_label_".$i, "", "raw");
				$message = JFactory::getApplication()->input->get("field_message_".$i, "", "raw");
				$cols = JFactory::getApplication()->input->get("field_cols_".$i, "", "raw") == "" ? "0" : JFactory::getApplication()->input->get("field_cols_".$i, "", "raw");
				$rows = JFactory::getApplication()->input->get("field_rows_".$i, "", "raw") == "" ? "0" : JFactory::getApplication()->input->get("field_rows_".$i, "", "raw");
				$option = JFactory::getApplication()->input->get("field_option_".$i, "", "raw") == "" ? "" : JFactory::getApplication()->input->get("field_option_".$i, "", "raw");
				$size = JFactory::getApplication()->input->get("field_size_".$i, "", "raw") == "" ? "0" : JFactory::getApplication()->input->get("field_size_".$i, "", "raw");
				...
				if(strpos($file_user_profile_content, 'name="'.trim($name).'"') === FALSE || strpos($file_user_profile_content, 'id="'.trim($id).'"') === FALSE){
					if(trim($option) != ""){
						...
						foreach($options_rows as $key=>$value){
							...
							$temp_user_text .= "\t"."\t"."\t"."\t".'<option value="'.trim(str_replace('&', '&amp;', $options["0"])).'">'.trim(str_replace('&', '&amp;', $options["1"])).'</option>'."\n";
						}					
			...
			foreach($fields["0"] as $key=>$value){				
				...
				if(isset($temp1) && count($temp1) > 0){					
					...
					foreach($temp1 as $key1=>$value1){						
						...
						elseif(strpos($value1, "spacer") !== FALSE){
							$add = false;
							break;
						}
	...
	function deleteFields(){
		...
		$name = JFactory::getApplication()->input->get("for_delete_name", "", "raw");
		$type = JFactory::getApplication()->input->get("for_delete_type", "", "raw");
