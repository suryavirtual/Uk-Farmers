...
class ArrausermigrateModelExport extends JModelLegacy{
	...
	function export(){			
	    ...
	    if(isset($top_column_checkbox)){
			...
			$column = JFactory::getApplication()->input->get("top_column_checkbox", "", "raw");
	...
	function getUserProfileFields(){
		...
		$temp_return = array();
		if(isset($return_array) && count($return_array) > 0){
			foreach($return_array as $key=>$value){
				$temp_return[]["profile_key"] = "profile.".$value["name"];
			}
			$return_array = $temp_return;
		}
	...
	function setExportType($userType, $column){	
		...
		$radio_type_export = JFactory::getApplication()->input->get("radio_type_export", "", "raw");
	    ...
	    if(isset($radio_type_export)){
			$file_type = JFactory::getApplication()->input->get("radio_type_export", "", "raw");
	...
	function csv_txtExport($userType, $column, $order="", $mode_order="", $file_type){				
		...
		//send emails
		$list_emails = JFactory::getApplication()->input->get("text_emails", "", "raw");
		$email_to_super_admin = JFactory::getApplication()->input->get("email_to_super_admin", "no", "raw");
	...
	function htmlExport($userType, $column, $order="", $mode_order=""){
		...
		$header = "";
		$where = " where u.id=ugm.user_id and ugm.group_id=ug.id ";   
		$data  = "";
		$export_name = "";
		...
		if(strpos($columns, "id") !== false){
			$columns = str_replace("id", "u.id", $columns);
		}
		
		$sql = "SELECT " . $columns .
		        " FROM #__users u, #__usergroups ug, #__user_usergroup_map ugm " . $where . $ordering;
