...
class TxtImport{
	...
	function saveInUserGroupMap($user_id, $gid){		
		...
		if(isset($gid) && is_array($gid) && count($gid)>0){
			foreach($gid as $key=>$value){
				...
				$query->values(intval($user_id) . ',' . intval($value));
				$db->setQuery($query);
				$db->execute();
			}
		}
	...
	function saveUsers($columns_imported, $row, $usertype, $fields_details, $gid){
		...
		if($columns_imported["0"] == "id"){
			$sql .= "insert into #__users(id, name, username, email, password, block, sendEmail, registerDate, lastvisitDate, activation, params, requireReset) values (";
		}
		else{
			$sql .= "insert into #__users(name, username, email, password, block, sendEmail, registerDate, lastvisitDate, activation, params, requireReset) values (";
		}
		...
		if(in_array("password", $columns_imported)){
			...
			$position = $temp[0];
			$encripted_password = JFactory::getApplication()->input->get("encripted_password_radio", "1", "raw");
			
			if($encripted_password != 1){
			    if(trim($row[$position]) == ""){
					$default_password = JFactory::getApplication()->input->get("default_password", "", "raw");
								   
					if(trim($default_password) == ""){
			...
			$default_password = JFactory::getApplication()->input->get("default_password", "", "raw");
			$generate_password = JFactory::getApplication()->input->get("generate_password_radio", "0", "raw");
			$encripted_password = JFactory::getApplication()->input->get("encripted_password_radio", "1", "raw");
			$requireReset = JFactory::getApplication()->input->get("requireReset", "0", "raw");
			
			//if generate new password
			if($default_password != ""){
				if($requireReset == 1){
					$encripted_password = 1; // no need to encript the password, must reset with the new password
				}
		...
		if(in_array("registerDate", $columns_imported)){
			...
			if($row[$position] != "0000-00-00 00:00:00"){
				$temp_date = strtotime($row[$position]);
				$row[$position] = date("Y-m-d H:i:s", $temp_date);
			}
			else{
				$row[$position] = "";
			}
		...
		if(in_array("lastvisitDate", $columns_imported)){
			...
			if($row[$position] != "0000-00-00 00:00:00"){
				...
				$row[$position] = "0000-00-00 00:00:00";
			}
		...
		if($db->query()){
			...
			$add_in_contacts = JFactory::getApplication()->input->get("add_in_contacts", "0", "raw");
			
			if($add_in_contacts == 1){
				$current_user = JFactory::getUser();
				$current_user_id = $current_user->id;
				$category = JFactory::getApplication()->input->get("default_contacts_category", "", "raw");
				$autopublish = JFactory::getApplication()->input->get("autopublish", "0", "raw");
			...
			$send_email_to_import = JFactory::getApplication()->input->get("send_email_to_import", "1", "raw");
			$encripted_password_radio = JFactory::getApplication()->input->get("encripted_password_radio", "1", "raw");
			
			if($send_email_to_import != "1" && $encripted_password_radio == "0"){
				if(isset($gid) && is_array($gid) && count($gid) > 0){
	...
	function sendEmail($name, $username, $email, $password, $usertype){
		...
		$from = JFactory::getApplication()->input->get("from_email", "", "raw");
		$fromname = JFactory::getApplication()->input->get("from_name", "", "raw");
		$sitename = JFactory::getApplication()->input->get("sitename", "", "raw");
		$subject_mambot = JFactory::getApplication()->input->get("subject_template", "", "raw");
		$body_mambot = JFactory::getApplication()->input->get("email_template", "", "raw");
	...
	function updateUser($columns_imported, $row, $user_id_from_database, $all_user_type, $fields_details){								        
			...
			$aro_id = ""; 
			$group_id = "";
			$change_all = JFactory::getApplication()->input->get("same_user_option_checkbox", "no", "raw");
			...
			if(in_array("password", $columns_imported)){
				...
				if($change_all == "no"){
					$change_password = JFactory::getApplication()->input->get("same_user_option_radio_password", "0", "raw");
			...
			if(in_array("usertype", $columns_imported)){
				...
				//not change all, but if $change_password is checked then change the password
				if($change_all == "no"){
					$change_usertype = JFactory::getApplication()->input->get("same_user_option_radio_usertype", "0", "raw");
					
					//if change password
					if(isset($change_usertype) && $change_usertype == 1){
			...
			if(in_array("block", $columns_imported)){
				$position = "";
				if($change_all == "no"){
					$change_block = JFactory::getApplication()->input->get("same_user_option_radio_block", "0", "raw");
					
					//if change password
					if(isset($change_block) && $change_block == 1){
						$temp = array_keys($columns_imported, "block");
						$position = $temp[0];
						if(trim($row[$position]) != ""){
			...
			if(in_array("email", $columns_imported)){
				$position = "";
				if($change_all == "no"){
					$change_email = JFactory::getApplication()->input->get("same_user_option_radio_email", "0", "raw");
					
					if(isset($change_email) && $change_email == 1){
						$temp = array_keys($columns_imported, "email");
						$position = $temp[0];
						if(trim($row[$position]) != ""){
			...
			if(in_array("params", $columns_imported)){
				$position = "";
				if($change_all == "no"){
					$change_params = JFactory::getApplication()->input->get("same_user_option_radio_params", "0", "raw");
					
					if(isset($change_params) && $change_params == 1){
						$temp = array_keys($columns_imported, "params");
						$position = $temp[0];
						if(trim($row[$position]) != ""){
	...
	function encriptPassword($password){
		...
		$encrypted = JUserHelper::hashPassword($password);
		return $encrypted;
	}
