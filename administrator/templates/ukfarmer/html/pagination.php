...
function pagination_item_active(&$item)
{
	...
	if (!is_numeric($item->text))
	{
		...
		if(!empty($_REQUEST['view'])){
			$url = JUri::current().'?option='.$_REQUEST['option'].'&view='.$_REQUEST['view'];
		} else{
			$url = JUri::current().'?option='.$_REQUEST['option'];
		}
		

		return '<li' . $class . '><a' . $title . ' href="'.$url.'&limitstart='.$limitnew.'" >' . $display . '</a></li>';
	}
