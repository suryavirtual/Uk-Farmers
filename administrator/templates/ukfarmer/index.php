...
$doc->addScriptVersion($this->baseurl . '/templates/' . $this->template . '/js/jquery.fixedheadertable.js');


// Add Stylesheets
$doc->addStyleSheetVersion($this->baseurl . '/templates/' . $this->template . '/css/template' . ($this->direction == 'rtl' ? '-rtl' : '') . '.css');
// Load custom.css
$file1 = 'templates/' . $this->template . '/css/fixedtableTheme.css';
...
if (is_file($file1))
{
	$doc->addStyleSheetVersion($file1);
}
...
/*jQuery(window).on('load', function () { 
        ...
        var stickyHeaderTop = $('.fixed_h2').offset().top;
 
        $(window).scroll(function(){
			console.log("window scroll-->"+$(window).scrollTop());
			console.log("stickyHeaderTop scroll-->"+stickyHeaderTop);
                if( $(window).scrollTop() > stickyHeaderTop ) {
                        $('.fixed_h2').css({position: 'fixed', top: '0px'});
                        //$('#stickyalias').css('display', 'block');
                } else {
                        $('.fixed_h2').css({position: 'static', top: '0px'});
 ...
 */
jQuery(window).on('load', function () {
	jQuery('#termList,#fileList,#emailList,#termList1,#emailList1').fixedHeaderTable({ footer: true, cloneHeadToFoot: true, altClass: 'odd', autoShow: true });
...
jQuery(window).on('resize', function () {
	jQuery('#termList,#fileList,#emailList,#termList1,#emailList1').fixedHeaderTable({ footer: true, cloneHeadToFoot: true, altClass: 'odd', autoShow: true });
      ...
      /*  jQuery(window).scroll(function(){
			var headWidth = jQuery('#adminForm > .fixed_top').width();
			//var headWidth1 = jQuery('#adminForm > .fixed_top1').width();

			//console.log("window scroll-->"+jQuery(window).scrollTop());
			//console.log("stickyHeaderTop1-------->"+headWidth1);
			//console.log("k-->"+stickyHeaderTop1);

			
			//console.log("stickyHeaderTop scroll for first header-->"+stickyHeaderTop);
			...
			if(k==0){
			 ...
			 if( jQuery(window).scrollTop() > k ) {
						jQuery('.headingss.fixed_h1').addClass('fix-heading').width(headWidth1 - 20);
						jQuery('.t1').addClass('fix-thead').width(headWidth1 - 20);
						
                } else {
						jQuery('.headingss.fixed_h1').removeClass('fix-heading');
						jQuery('.t1').removeClass('fix-thead');
						
                }
			
                if( jQuery(window).scrollTop() > j ) {
						jQuery('.headingss.fixed_h2').addClass('fix-heading').width(headWidth - 20);
						jQuery('.t2').addClass('fix-thead').width(headWidth - 20);
                } else {
						jQuery('.headingss.fixed_h2').removeClass('fix-heading');
						jQuery('.t2').removeClass('fix-thead');
                }
        });*/
 	</script>
</head>
...
<nav class="navbar<?php echo $template_is_light ? '' : ' navbar-inverse'; ?> navbar-fixed-top">
	<div class="navbar-inner">
		<div class="container-fluid">
			...
			<a class="brand hidden-desktop hidden-tablet" href="<?php echo JUri::root(); ?>" title="<?php echo JText::sprintf('TPL_UKFARMER_PREVIEW', $sitename); ?>" target="_blank"><?php echo JHtml::_('string.truncate', $sitename, 14, false, false); ?>
					...
					<li class="dropdown">
						...
						<ul class="dropdown-menu">
							<?php if (!$hidden) : ?>
								...
								<li class="divider"></li>
								<li>
									<a href="index.php?option=com_admin&amp;task=profile.edit&amp;id=<?php echo $user->id; ?>"><?php echo JText::_('TPL_UKFARMER_EDIT_ACCOUNT'); ?></a>
								</li>
								<li class="divider"></li>
								<li class="">
									<a href="<?php echo JRoute::_('index.php?option=com_login&task=logout&' . JSession::getFormToken() . '=1'); ?>"><?php echo JText::_('TPL_UKFARMER_LOGOUT'); ?></a>
								</li>
							<?php endif; ?>
						</ul>
					</li>
				</ul>
				<a class="brand visible-desktop visible-tablet" href="<?php echo JUri::root(); ?>" title="<?php echo JText::sprintf('TPL_UKFARMER_PREVIEW', $sitename); ?>" target="_blank"><?php echo JHtml::_('string.truncate', $sitename, 14, false, false); ?>
					<span class="icon-out-2 small"></span></a>
			</div>
			<!--/.nav-collapse -->
		</div>
...
<?php if (!$cpanel) : ?>
	<!-- Subheader -->
	<a class="btn btn-subhead" data-toggle="collapse" data-target=".subhead-collapse"><?php echo JText::_('TPL_UKFARMER_TOOLBAR'); ?>
